...
Core Principles
Unique value per page — Every page must provide value specific to that page, not just swapped variables in a template
Proprietary data wins — Hierarchy: proprietary > product-derived > user-generated > licensed > public (weakest)
Subfolders, not subdomains — yoursite.com/templates/resume/ not templates.yoursite.com/resume/
Match search intent — Pages must actually answer what people are searching for
...
Study the main app's header/footer colors, logo treatment, link structure, and typography
Replicate them in the shared ssrShared.ts shell functions — not approximations, pixel-for-pixel matches
When the main app's footer is updated (new links, new columns), update the SSR footer function too
Conversely, when new SSR hub pages are added (e.g., /glossary, /guides), add them to the main app's footer navigation
...
Rendering audit: Confirm whether the app is a SPA and what SSR approach to use
Strategy: Opportunity analysis, chosen playbook(s), keyword patterns, data sources, page count estimate
Shared infrastructure: ssrShared.ts (or equivalent) HTML shell before any individual pages
Template: URL structure, title/meta templates, content outline, schema markup
Implementation: Working server-rendered pages that return full HTML to curl
Integration: Footer/nav updates in the main app linking to the new hub pages
Limitations
