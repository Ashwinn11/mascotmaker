                ...
                <div className="grid md:grid-cols-3 gap-24">
                    <div className="space-y-16">
                        <div className="space-y-10">
                            ...
                            <div className="grid gap-4">
                                {ENGINES.map((e) => (
                                    <Link key={e.slug} href={`/mascot-maker/use-case/${e.slug}`} className="group flex items-center gap-4 text-xl font-black text-foreground/60 hover:text-candy-pink transition-all">
                        ...
                        <div className="space-y-10">
                            ...
                            <div className="grid gap-3">
                                {sortedIndustries.map((i) => (
                                    <Link key={i.slug} href={`/mascot-maker/industry/${i.slug}`} className={`text-sm font-bold uppercase tracking-widest hover:text-candy-blue transition-colors ${i.slug === "ai-startups" ? "text-candy-blue" : "text-muted-foreground"}`}>
                                        • {i.title} Assets
                                    </Link>
                                ))}
                            </div>
                    ...
                    <div className="space-y-16">
                        <div className="space-y-10">
                            ...
                            <div className="grid gap-3">
                                {STYLES.map((s) => (
                                    <Link key={s.slug} href={`/mascot-maker/style/${s.slug}`} className="text-sm font-bold uppercase tracking-widest text-muted-foreground hover:text-candy-green transition-colors">
                                        • {s.title} Design
                                    </Link>
                                ))}
                            </div>
                    ...
                    <div className="space-y-16">
                        <div className="space-y-10">
                            <h2 className="font-display text-4xl uppercase tracking-tight text-foreground/80">Style × Industry</h2>
